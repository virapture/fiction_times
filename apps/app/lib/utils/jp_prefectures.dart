/// 変更可能なように以下から持ってきた
/// https://github.com/yhirano/flutter_jp_prefecture/blob/main/lib/jp_prefecture.dart
class JpPrefecture {
  JpPrefecture(
    this._code,
    this._name,
  ...
  factory JpPrefecture.fromJson(Map<String, dynamic> json) {
    return JpPrefecture(
      json['code'] as int? ?? 0,
      json['name'] as String? ?? '',
      json['nameE'] as String? ?? '',
      json['nameH'] as String? ?? '',
      json['nameK'] as String? ?? '',
      json['area'] as String? ?? '',
    );
  }
  ...
  /// Get all prefectures.
  static Iterable<JpPrefecture> get all => data.values;
  ...
  static int? _findCodeByName(String name) {
    ...
    for (final jpPrefecture in data.values) {
  ...
  static final Map<int, JpPrefecture> data = {
...
extension JpPrefectureAreaExtension on JpPrefectureArea {
  String get name {
    switch (this) {
      case JpPrefectureArea.hokkaido:
        return '北海道';
      case JpPrefectureArea.tohoku:
        return '東北';
      case JpPrefectureArea.kanto:
        return '関東';
      case JpPrefectureArea.chubu:
        return '中部';
      case JpPrefectureArea.kinki:
        return '関西';
      case JpPrefectureArea.chugoku:
        return '中国';
      case JpPrefectureArea.shikoku:
        return '四国';
      case JpPrefectureArea.kyushu:
        return '九州';
    }
  }

  List<JpPrefecture> get prefectures {
    return JpPrefecture.data.entries
        .where((element) => element.value.area == name)
        .map((e) => e.value)
        .toList();
  }
}
